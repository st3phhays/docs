...
import ChocolateyComponentDependenciesLink from '@components/docs/ChocolateyComponentDependenciesLink.astro';
import MaintenanceAndSupportLink from '@components/docs/MaintenanceAndSupportLink.astro';
...
With this release, Package Parameters are fully supported from both the user side and the packaging side. Check out <Xref title="the documentation](https://docs.chocolatey.org/en-us/create/functions/get-packageparameters) and check out the [walkthrough" value="parse-package-parameters" /> on how to use package parameters in your packaging. Note if you are pushing packages to the community repository, you must continue to take a dependency on the `chocolatey-core.extension` as a polyfill for older versions until at least six months after a new feature is released.
